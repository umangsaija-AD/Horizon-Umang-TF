...
class LocalizationFormComponent extends Component {
  ...
  #onContainerKeyDown = (event) => {
    ...
    switch (event.key) {
      ...
      case 'Enter': {
        ...
        if (focusedItem) {
          const nextValue = focusedItem.dataset.value ?? '';
          this.#clearCartIfCountryChanged(countryInput, nextValue);
          countryInput.value = nextValue;
          if (nextValue === 'US') this.#forceEnglish(form);
          form.submit();
        }
        break;
      }
  ...
  selectCountry = (countryName, event) => {
    ...
    this.#clearCartIfCountryChanged(countryInput, countryName);

    countryInput.value = countryName;

    // The United States is English-only for this store — force the language to
    // English when switching to the US so any non-English locale (e.g. fr) is
    // dropped from the resulting URL.
    if (countryName === 'US') this.#forceEnglish(form);

    form?.submit();
  };

  /**
   * Forces the localization form to submit with English selected. Used when the
   * shopper switches to the United States: it sets the language select to the
   * English option (creating one if the current market's options don't include
   * it) or, when no language select is rendered, appends a hidden language_code
   * input. The result drops any non-English locale prefix from the URL.
   *
   * @param {HTMLFormElement} [form] - The localization form.
   */
  #forceEnglish(form) {
    const { languageInput } = this.refs;

    if (languageInput) {
      let match = Array.from(languageInput.options).find(
        (option) => option.value.toLowerCase() === 'en' || option.value.toLowerCase().startsWith('en-')
      );
      if (!match) {
        match = new Option('English', 'en');
        languageInput.add(match);
      }
      languageInput.value = match.value;
      return;
    }

    if (!form) return;
    let hidden = form.querySelector('input[name="language_code"]');
    if (!hidden) {
      hidden = document.createElement('input');
      hidden.type = 'hidden';
      hidden.name = 'language_code';
  ...
  /**
   * Clears the cart cookie when the shopper switches to a different country, so
   * the new locale starts with an empty cart (prices/availability differ per
   * market). No-op when the selected country matches the current one.
   *
   * @param {HTMLInputElement} countryInput - The hidden country input holding the current ISO code.
   * @param {string} nextCountry - The ISO code being switched to.
   */
  #clearCartIfCountryChanged(countryInput, nextCountry) {
    if (!countryInput || countryInput.value === nextCountry) return;
